    ...
    if user_signed_in?
      ...
      if current_user.profile.nil? 
        if !params[:search_instrument].nil? 
          @profiles = @profiles.where("profiles.primary_instrument.downcase LIKE ?", "%#{params[:search_instrument.downcase]}%")
        end
        if !params[:search_name].nil? 
          @profiles = @profiles.where("profiles.name.downcase LIKE ?", "%#{params[:search_name.downcase]}%")
        end
        if !params[:search_city].nil? 
          @profiles = @profiles.where("profiles.city.downcase LIKE ?", "%#{params[:search_city.downcase]}%")
        end
        if !params[:search_state].nil? 
          @profiles = @profiles.where("profiles.state.downcase LIKE ?", "%#{params[:search_state.downcase]}%")
        end
      else 
         if !params[:search_instrument].nil? 
           @users = @users.where("profiles.primary_instrument.downcase LIKE ?", "%#{params[:search_instrument.downcase]}%")
        end
        if !params[:search_name].nil? 
          @users = @users.where("profiles.name.downcase LIKE ?", "%#{params[:search_name.downcase]}%")
        end
        if !params[:search_city].nil? 
          @users = @users.where("profiles.city.downcase LIKE ?", "%#{params[:search_city.downcase]}%")
        end
        if !params[:search_state].nil? 
          @users = @users.where("profiles.state.downcase LIKE ?", "%#{params[:search_state.downcase]}%")
        end
      end
    else 
      if !params[:search_instrument].nil? 
